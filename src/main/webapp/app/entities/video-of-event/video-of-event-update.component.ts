...
export default class VideoOfEventUpdate extends Vue {
    ...
    if (this.videoOfEvent.id) {
      this.videoOfEventService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('platemillApp.videoOfEvent.updated', { param: param.id });
      ...
      this.videoOfEventService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('platemillApp.videoOfEvent.created', { param: param.id });
